...
namespace X_Wcf.Administrator.BasePlatform.Role
{
    public partial class Default : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                btnAdd.OnClientClick = windowSourceCode.GetShowReference("Add.aspx", "添加角色信息");
        ...
        private DataSet InitGrid()
        {
            ...
            source = new DataSet();
            source = manage.GetSearchList(ViewState["Search"].ToString());
            return source;
        }
        ...
        protected void Grid1_RowDataBound(object sender, FineUI.GridRowEventArgs e)
        {
            ...
            if (row != null)
            {
                if (e.Values[0].ToString() == "0")
                {
                    e.Values[0] = "平台角色";
                }
                if (e.Values[0].ToString() == "1")
                {
                    e.Values[0] = "应用角色";
                }
