...
namespace X_Wcf.Administrator.BasePlatform.Account
{
    public partial class Default : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                btnAdd.OnClientClick = windowSourceCode.GetShowReference("Add.aspx", "添加用户信息");
        ...
        protected void Grid1_RowDataBound(object sender, FineUI.GridRowEventArgs e)
        {
            ...
            if (row != null)
            {
                if (e.Values[5].ToString() == "0")
                {
                    e.Values[5] = "<span style='color:red'>停用</span>";
                }
                if (e.Values[5].ToString() == "1")
                {
                    e.Values[5] = "<span style='color:green'>启用</span>";
                }
            }
