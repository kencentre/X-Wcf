...
namespace OracleDAL
{
	...
	public  class Notice:E_IDAL.INotice
	{
        ...
        public DataSet GetSearchList(string strWhere, string startDate, string endDate)
        {
            ...
            if (startDate != "" && endDate != "")
            {
                strSql.Append(" and SYS_NOTICE.createdate between to_date('" + startDate + " 00:00:00','yyyy-MM-dd hh24:mi:ss') and to_date('" + endDate + " 23:59:59','yyyy-MM-dd hh24:mi:ss')");
            }
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select *  FROM SYS_NOTICE where status = '1' ");
            if (strWhere.Trim() != "")
            {
                strSql.Append("  and  (SYS_NOTICE.TITLE like '%" + strWhere + "%' or SYS_NOTICE.KEYWORD like '%" + strWhere + "%' ) ");
            }

            strSql.Append(" and ROWNUM <= " + rowNum + " order by createdate desc ");
            Database db = DatabaseFactory.CreateDatabase();
            return db.ExecuteDataSet(CommandType.Text, strSql.ToString());
        }
        #endregion  Method
