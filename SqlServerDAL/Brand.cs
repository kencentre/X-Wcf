...
{
    public class Brand:E_IDAL.IBrand
    {
        public Brand()
        { }
        ...
        /// 增加一条数据
        /// </summary>
        public bool Add(E_Model.Brand model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into CMS_BRAND(");
            strSql.Append("ID,BRANDNAME,BRANDENAME,SHOWPICTURE,SLOGAN,STATUS,DESCRIPTION,CREATEDATE,CREATEUSER)");
            ...
            db.AddInParameter(dbCommand, "BRANDENAME", DbType.AnsiString, model.BRANDENAME);
            db.AddInParameter(dbCommand, "SHOWPICTURE", DbType.AnsiString, model.SHOWPICTURE);
            db.AddInParameter(dbCommand, "SLOGAN", DbType.AnsiString, model.SLOGAN);
        ...
        /// 更新一条数据
        /// </summary>
        public bool Update(E_Model.Brand model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update CMS_BRAND set ");
            strSql.Append("BRANDNAME=@BRANDNAME,");
            strSql.Append("BRANDENAME=@BRANDENAME,");
        ...
        public bool Delete(string ID)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from CMS_BRAND ");
            strSql.Append(" where ID=@ID ");
            Database db = DatabaseFactory.CreateDatabase();
            DbCommand dbCommand = db.GetSqlStringCommand(strSql.ToString());
        ...
        /// 得到一个对象实体
        /// </summary>
        public E_Model.Brand GetModel(string ID)
        {

            StringBuilder strSql = new StringBuilder();
            strSql.Append("select ID,BRANDNAME,BRANDENAME,SHOWPICTURE,SLOGAN,STATUS,DESCRIPTION,CREATEDATE,CREATEUSER from CMS_BRAND ");
            strSql.Append(" where ID=@ID ");
            Database db = DatabaseFactory.CreateDatabase();
            DbCommand dbCommand = db.GetSqlStringCommand(strSql.ToString());
            db.AddInParameter(dbCommand, "ID", DbType.AnsiString, ID);
            E_Model.Brand model = null;
        ...
        public E_Model.Brand DataRowToModel(DataRow row)
        {
            ...
            if (row != null)
            {
                ...
                if (row["BRANDENAME"] != null)
                {
                    model.BRANDENAME = row["BRANDENAME"].ToString();
                }
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select ID,BRANDNAME,BRANDENAME,SHOWPICTURE,SLOGAN,STATUS,DESCRIPTION,CREATEDATE,CREATEUSER ");
            strSql.Append(" FROM CMS_BRAND ");
            if (strWhere.Trim() != "")
            {
        ...
        public DataSet GetListByPage(string strWhere, string startDate, string endDate, string startIndex, string endIndex)
        {
            ...
            strSql.Append(")AS Row, T.*  from CMS_BRAND T where 1=1  ");
            if (!string.IsNullOrEmpty(strWhere.Trim()))
            {
                strSql.Append(" AND BRANDNAME like '%"+ strWhere + "%' OR BRANDENAME like '%" + strWhere + "%'" );
            }
        ...
        /// </summary>
        public List<E_Model.Brand> GetListArray(string strWhere)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select ID,BRANDNAME,BRANDENAME,SHOWPICTURE,SLOGAN,STATUS,DESCRIPTION,CREATEDATE,CREATEUSER ");
            strSql.Append(" FROM CMS_BRAND ");
            ...
            List<E_Model.Brand> list = new List<E_Model.Brand>();
            Database db = DatabaseFactory.CreateDatabase();
        ...
        /// 对象实体绑定数据
        /// </summary>
        public E_Model.Brand ReaderBind(IDataReader dataReader)
        {
            E_Model.Brand model = new E_Model.Brand();
            object ojb;
            model.ID = dataReader["ID"].ToString();
            model.BRANDNAME = dataReader["BRANDNAME"].ToString();
            model.BRANDENAME = dataReader["BRANDENAME"].ToString();
            model.SHOWPICTURE = dataReader["SHOWPICTURE"].ToString();
            model.SLOGAN = dataReader["SLOGAN"].ToString();
